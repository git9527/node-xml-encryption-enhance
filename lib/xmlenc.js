...
function encryptKeyInfoWithScheme(symmetricKey, options, scheme, callback) {
  ...
  try {
    ...
    if (options.keyInfoTemplate) {
      var result = options.keyInfoTemplate(params)
    } else {
...
function decryptKeyInfoWithScheme(encryptedKey, options, scheme) {
  ...
  var encryptedContent = Buffer.from(encryptedKey.textContent, 'base64');
  var decryptionOptions = { key: options.key, padding: padding}
  if (options.decryptKeyInfo) {
    var decrypted = options.decryptKeyInfo(decryptionOptions, encryptedContent)
  } else {
    var decrypted = crypto.privateDecrypt(decryptionOptions, encryptedContent);
  }
